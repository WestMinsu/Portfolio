...
(function ($) {
    ...
    const translations = {
        ...
        <ul>
            <li><strong>Chaos Destruction 활용</strong><br> 언리얼 엔진의 Chaos Destruction 시스템을 활용하여 총알에 맞았을 때 파괴되는 벽을 구현했습니다. GeomertryCollection의 파괴된 조각이 Replication되지 않는 문제가 있었는데, 엔진 내부 코드를 살펴보며 부서진 조각을 Replication하는 함수의 버전이 2개임을 확인하여 이전 버전의 함수를 사용함으로써 문제를 해결했습니다.</li><br>
